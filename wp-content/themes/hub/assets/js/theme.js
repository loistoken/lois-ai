  ...
  class Plugin {
    ...
    init() {
      ...
      $triggers.each((i, trigger) => {
        ...
        this.checkPreventDefault($trigger);
      });
    }

    checkPreventDefault($trigger) {
      const {
        triggerHandlers
      } = this.options;

      if ((triggerHandlers[0] === 'mouseenter' || triggerHandlers[0] === 'pointerenter') && $trigger[0].hasAttribute('href') && $trigger[0].getAttribute('href') === '#') {
        $trigger.on('click', e => {
          e.preventDefault();
        });
      }
  ...
  class Plugin {
    ...
    getTargetsArray(itemIndex, item) {
      if (this.options.scrollBelowSection) {
        const $parentRow = liquidIsElementor ? this.$element.parents('.elementor-section, .e-container').last() : this.$element.parents('.vc_row, .vc_section').last();
        const $nextRow = liquidIsElementor ? $parentRow.nextAll('.elementor-section, .e-container').first() : $parentRow.nextAll('.vc_row, .vc_section').first();
