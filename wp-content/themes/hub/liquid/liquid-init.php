...
class Liquid_Theme extends Liquid_Base {
	...
	public function admin_update_redirect() {
		...
		if ( version_compare( $theme_version, '2.0', '>=' ) ){
			...
			if ( 
				($tgmpa->is_plugin_active( 'hub-elementor-addons' ) && version_compare( LD_ELEMENTOR_VERSION, '3.0.1', '<' )) ||
				( 
				  ( class_exists('Liquid_Addons') && !defined('LD_ADDONS_VERSION') ) || 
				  ( class_exists('Liquid_Addons') && (defined('LD_ADDONS_VERSION') && version_compare( LD_ADDONS_VERSION, '3.0', '<' )) ) 
				) 
			){
